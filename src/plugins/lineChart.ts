...
class SeriesSegmentVertexArray {
    ...
    popBack(n: number): number {
        ...
        return this.validStart / POINT_PER_DATAPOINT - newVaildEndDP;
    }

    popFront(n: number): number {
        const newVaildStart = this.validStart + n * POINT_PER_DATAPOINT;
        this.validStart = Math.min(newVaildStart, this.validEnd);
        return (newVaildStart - this.validEnd) / POINT_PER_DATAPOINT;
    }
...
class SeriesVertexArray {
    ...
    private popFront() {
        ...
        let activeArray = this.vertexArrays[0];
        while (true) {
    ...
    draw(renderDomain: { min: number, max: number }) {
        ...
        const minArrayIndex = Math.floor((minIndex + offset) / BUFFER_DATA_POINT_CAPACITY);
        const maxArrayIndex = Math.ceil((maxIndex + offset) / BUFFER_DATA_POINT_CAPACITY);

        const renderIndex = { min: minIndex, max: maxIndex };
        for (let i = minArrayIndex; i < maxArrayIndex; i++) {
            offset += this.vertexArrays[i].draw({
                min: renderIndex.min + offset,
                max: renderIndex.max + offset,
            });
        }
    }
}
